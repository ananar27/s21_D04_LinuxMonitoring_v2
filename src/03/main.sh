...
function delete_by_time() {
  ...
  # Для эффективной демонстрации необходимо специфицировать путь до директории в которой проходит тестирование, 
  # иначе можно очень долго прождать, пока скрипт пройдется по всей системе
  # find /{path} (на текущей тачке папка для тестирования срипта лежит в /home/mel/02)
...
function main() {
  if [[ $# -ne 1 ]]; then
    echo "To use this script you have to pass it a method as an argument.\n Use 1-3 to choose a method."
    exit 1
  fi
  ...
  if [[ method -eq 1 ]]; then 
    ...
    echo "Invalid method. Use options 1-3."
    exit 1
  fi
}
